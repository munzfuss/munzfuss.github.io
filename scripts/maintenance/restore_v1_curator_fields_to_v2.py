...
def _has_real_note(coin: dict) -> bool:
    """True when coin.note has non-empty content. Treats None / empty
    dict / dict-with-all-empty-values as «no note»."""
    n = coin.get("note")
    if not n:
        return False
    if isinstance(n, dict):
        return any(v and str(v).strip() for v in n.values())
...
def _restore_one(v1_coin: dict, v2_coin: dict) -> dict:
    ...
    # Note (curator prose) — propagate ONLY when V2 has NO note. When
    # V2 has its own note (e.g. auto-generated by absorb's enrich path
    # or curator-written post-V1), leave it untouched (curator-decision
    # territory — see audit 2026-05-27: 95 dual-note cases are mostly
    # V1↔V2 duplicates already, no propagation needed).
    if _has_real_note(v1_coin) and not _has_real_note(v2_coin):
        v2_coin["note"] = v1_coin["note"]
        counts["note"] = 1
...
def main() -> int:
    ...
    print(f"Notes restored:          {grand['note']}")
