...
>
> **In-flight refactor: V2 entity-keyed pipeline — `docs/V2_PIPELINE.md`.** A multi-session refactor that re-keys Phase 3 (SEED) and Phase 4 (CURATED) data files from `<location>.yml` to `<political_entity>.yml`, with display pages declaring `consumes_entities: [...]` and Phase 5 (MERGED) assembling per-page from N entity files. Plan approved in principle (2026-05-18) but **not yet started**; V1 remains source of truth until the user signals «фліпай V2». Read `docs/V2_PIPELINE.md` before touching seed-file organisation, `_merge_seeds_into_raw`, the cross-location coverage hack, or anything that hard-codes the location-keyed assumption — your change may collide with the planned restructure. Atomic-commit cadence applies per phase; V2 work lives on branch `feat/v2-pipeline`.
