...
5. **Writing «cf. X» refs into catalog index columns.** A «cf.» reference (`KM-cf. 15`, `Lange-cf. 264a`, `lange: cf. 445`, `KM-unlisted (cf. 136)`) points at a similar OTHER coin, not at the entry's own catalogue index — it doesn't belong in `catalog.km / hede / sieg / lange / fr / dav / others`. When the cf-relation is historically load-bearing (Bruun's auction quote names it as the nearest comparable type), keep it in `note` prose framed as «closest-related reference is X (cf.)» — never as a positive catalog field for THIS coin. Per V2_DECISIONS D31 the merger filters cf-form values at ingest so no future seed re-introduces them; if you encounter a cf-form value in a catalog field anywhere, it's a regression — strip it and check why the filter missed it.
