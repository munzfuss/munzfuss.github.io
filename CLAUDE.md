...
**Numista- and ucoin.net-specific:** both catalogue sites (Cloudflare-protected, frequent 403 from WebFetch/Apify) are **not** sanctioned scraping targets via Chrome MCP, browser console, or any other browser-automation route. For Numista, use the official API exclusively (see «Numista API budget» below); ucoin.net has no public API at all for the numismatic catalogue. For both sites, when the available paths are exhausted (Numista API quota burnt; ucoin WebFetch returns 403), ask the user to paste the relevant page content into chat — do **not** spin up Chrome MCP to fetch the HTML. The fallback chain above (WebSearch / WebFetch / Apify / Chrome MCP) stays valid for OTHER public sources; Numista and ucoin.net are the explicit exceptions.

**Pattern for browser automation via Chrome MCP:** for sources other than Numista and ucoin.net — always batch actions with `browser_batch` (the runtime gives a system-reminder if you don't). Sequence: `list_connected_browsers` → `select_browser` → `tabs_context_mcp(createIfEmpty: true)` → then a single `browser_batch` with `[navigate, get_page_text]` per URL. For multi-page comparisons, one `browser_batch` per URL keeps the round trip count low.
