...
>
> **In-flight refactor: V2 entity-keyed pipeline — `docs/V2_PIPELINE.md`.** A multi-session refactor that re-keys Phase 3 (SEED) and Phase 4 (CURATED) data files from `<location>.yml` to `<political_entity>.yml`, with display pages declaring `consumes_entities: [...]` and Phase 5 (MERGED) assembling per-page from N entity files. **Status (2026-05-18):** Phases 0, 1, 2, 4 (build-pipeline / assembly), 5 (schema + resolvers) landed in `data/v2/` + `scripts/lib/v2_resolver.py`. V2 currently renders 12 location pages at `site/v2/<loc>/<lang>/` alongside V1 at `site/<loc>/<lang>/` — V1 remains the live source of truth until the user signals «фліпай V2». **Pending:** Phase 3 (entity-keyed seed builders for hede / bruun / galster / numista / numismaster — V2 still has zero seed coins), Phase 6 (bidirectional seed↔curated link), Phase 7 (`audit_v2.py` — hard-block from this phase per user §7.4 decision). Phase 8 (user review iteration) and Phase 9 (promotion-to-main, archive V1) come last. Read `docs/V2_PIPELINE.md` before touching seed-file organisation, `_merge_seeds_into_raw`, the cross-location coverage hack, the V2 `_assemble_v2_location()` assembly function (`scripts/build.py`), the `lib/v2_resolver.py` resolvers, or anything that hard-codes the location-keyed assumption. Atomic-commit cadence applies per phase; V2 work lives on branch `feat/v2-pipeline` (the worktree branch `claude/sleepy-murdock-593353` serves as functional equivalent for this branch).
