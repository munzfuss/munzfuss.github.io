...
**⚠️ Propagation BLOCKED — absorb non-idempotency (found 2026-05-30, NOT shipped).** Re-running merger + absorb `--entity` for the 10 IKMK entities propagated 258 IKMK coins to final BUT **de-promoted 9 pre-existing coins** (danish_realm 4, danish_norway 2, royal_holstein 3 — e.g. `unified-denmark-numismaster-65634/65627/65629`). They survive in seed + seed_unified but the re-`absorb` did not re-promote them to final (they were promoted in HEAD). So a straight re-run regresses 3 rendered pages. **The propagation was reverted.** Before IKMK can flow to final/render, the absorb de-promotion must be understood + guarded (either absorb idempotency — preserve existing-promoted coins across re-runs — or per-coin `bulk_promote` decisions for the de-promoted set). This likely affects ANY absorb re-run, not just IKMK (general pipeline concern). Until then IKMK stays seed-only (counted in `harvest_coverage.py` §A, not on pages).
