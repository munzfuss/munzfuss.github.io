...
### CJ-IKMK. 🟡 IKMK prose-citation catalogue capture (German-states catalogues)  *(opened 2026-06-01, split from §CJ)* *(est: medium)* *(type: parser enhancement — needs decision)* — **needs design**

IKMK `literatur` is prose bibliography, not code+number tokens. The current 6-author white-list (Hede/Davenport/Lange/Sieg/Schou/Friedberg) misses German-states catalogues cited by author surname mid-citation (Schön, Schrötter, Olding, Duve, Fiala, Noss, …) — relevant for the German entities (Lauenburg, Braunschweig-Lüneburg, etc.). Options: (a) extend the author→code map with the recurrent German-states authors (curatorial, bounded list — most-frequent leading surnames are enumerable from the cache), routing each to `others` «Author# Nr»; (b) leave as-is (Danish-core captured, German-states refs are lower-value for the mission). Decide before any IKMK→final propagation that would surface German-entity coins.
...
**Surfaced.** 2026-06-01. Seed-level changes accumulated that are NOT yet on the rendered pages: (a) numista index-fix re-seed — **947 numista seeds gained `others`/`dav`-volume refs** (`0c84510` + `_ALLOWED_CATALOG`-from-schema), (b) §CM German-entity ucoin coins once routed, (c) §CJ Bruun **Aagaard recovery (243 refs → `others`)** + Galster/Hede generic-mapping (lossless but re-serialises). Run the single coordinated propagation: `merge_seeds_cross_source.py --apply` (all entities) → `gen_overmerge_purge_allowlist.py --write` → `absorb_seeds_into_final_v2.py --apply` (all entities) → `build.py`. Verify: over-merge audit SAFE 0, no new dups, all curator `merge_decisions`/`no_merges` reapply, the recovered numista indices + §CM ucoin coins + §CJ Aagaard refs surface, build clean. Cache-only, no API cost; large run touching all entities.

> **§CL MUST regenerate bruun/galster/hede seeds first** (`build_{bruun,galster,hede}_denmark_seed.py`, cache-only) — §CJ landed code-only, so those seeds are stale vs their builders. Expect a LARGE but legitimate seed diff combining: §CJ Aagaard recovery (+`others`), pre-existing §CG note-annotation drift (note/nominal/translation churn — confirmed builder-driven, not hand-edits), and a Galster/Hede entry re-indent to the current `coins:`-nested writer form. Commit that seed regen as its own atomic step (separate from the merge/absorb/render commits) so the §CG drift + §CJ recovery are reviewable together but isolated from the propagation logic.
