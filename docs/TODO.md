...
+ **Friedrich/Frederik NAME fold — STILL OPEN, DEFERRED («імена потім», user 2026-06-03):** German «Friedrich» vs Danish «Frederik» NAME spelling (distinct from the numeral, now done). RISKY: German «Friedrich» rulers are mostly DIFFERENT people (Friedrich Wilhelm III Prussia, Johann Friedrich I Saxony 1535, Friedrich Ulrich Braunschweig — `friedrich iii` alone spans 1491-1888 = ≥3 persons), NOT Danish Frederik kings — a blanket friedrich↔frederik fold would be wrong + low-value. Only the Bremen-Verden archbishop «Johann Friedrich»/«John Frederik»/«Ærkebisp Johann Friedrich» genuinely needs an entity-scoped fold (+ ærkebisp-title strip). Do per-entity + per-reign-window, NOT blanket. Likely also Christian Albrecht/Christian Albert (Gottorp). Curator-directed, after the numeral work settles.
