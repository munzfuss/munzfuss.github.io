...
+ **Friedrich/Frederik ruler fold — STILL OPEN (riskier, needs care):** «Johann Friedrich»/«Johan Friedrich»/«John Frederik»/«Johan Frederik» + the `Ærkebisp Johann Friedrich`(14) title-prefixed form span MULTIPLE distinct rulers — Johann Friedrich I of Saxony (1535-1553), the Bremen-Verden archbishop (1596-1679), Braunschweig-Lüneburg — whose reign windows DON'T cleanly separate. Per-entity matcher helps (within bremen_verden the Friedrich forms are likely one archbishop), but a blanket Friedrich↔Frederik fold is unsafe. Needs per-entity + per-reign-window survey + an `ærkebisp`-title strip (deliberately NOT added with the ducal-title strip). Likely also Christian Albrecht/Christian Albert (Gottorp). Curator-directed, per-pair.
