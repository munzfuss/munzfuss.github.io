...
### CO. 🟡 `german_empire` entity decision — 27 ucoin coins unmapped → don't render  *(opened 2026-06-01, user-directed «високий пріоритет»)* *(est: small)* *(type: curator decision + builder mapping)* — **needs decision**

The 27 `german_empire` ucoin coins (generic Reichsmark / unified-Empire common types) are NOT mapped to any project entity (`build_ucoin_seed.py::URL_COUNTRY_TO_ENTITY` deliberately skips `german_empire` — §CM left it as a curator call). They're harvested in cache but never seeded → never rendered. Decide: which mission entity (if any) owns the generic Reichsmark types? Options: (a) none — out of scope (unified-Empire coinage isn't a single mission state); (b) split per mint/issuer into the existing German entities; (c) a new `german_empire` location for the Reichswährung tier. If (b)/(c): add the mapping to `URL_COUNTRY_TO_ENTITY` + `ENTITY_WINDOW` + (for c) a display location yaml, then re-seed.
