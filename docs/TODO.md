...
### CG. 🟢 §1 nominal hygiene — strip embedded equivalents / fractions / weights / nicknames out of the `nominal` field  *(opened 2026-05-30)* *(est: medium-large)* *(type: data-audit + curation)*

**Surfaced.** 2026-05-30, during the Nobel-spelling normalisation work (V2_DECISIONS D43). User clarification of CLAUDE.md §1: **what the coin actually IS matters — there's a difference between a coin with NO inscribed nominal vs one WITH an inscribed nominal. If the nominal is inscribed, THAT inscription is the `nominal` in our table — never an equivalent (e.g. not «1 Krone = 4 Mark»). If NOT inscribed, we may use what the source(s) provide.** Equivalents / fractions / weights / nicknames belong in `note` (or the proper structured field), never in `nominal`.

**Finding.** ~50 V2 `nominal` fields carry non-inscription content embedded in the denomination string. Splits by sub-pattern:

- **Weight-in-parens (parser bleed — the number is grams, NOT a count):** `1 Ducat (3.5)`, `1 Goldgulden (1.5)`, `1 Goldgulden (3.25)`, `1 Crown (15.75)`, `1 Ducat (Dukat) (3.5)`. → drop from nominal (weight already lives in `weight_rough_g`). NO legend check needed.
- **Fraction-of-standard:** `1 Groschen (1/24 Thaler)`, `1 Pfennig (1/288 Thaler)`, `1 Grote (1/72 Thaler)`, `1 Hvid (1/144 Speciedaler)`, `1 Skilling (1/96 Speciedaler)`, `1 Mariengroschen (1/36 Thaler)`, `1 Schwaren (1/360 Thaler)`, `12 Pfennige (1/21 Thaler)`, `12 Grote (1/6 Thaler)`. → fraction → `note` / `fraction` field. NO legend check.
- **Nickname / Latin / type:** `1 Skilling (hulpenning)`, `1 Speciedaler (Prinsens Daler)`, `2 Reichsthaler (Doppeltaler)`, `6 Mark (Rejsedaler)`, `3 Skilling Lybsk (Døtgen)`, `½ Skilling (Søsling)`, `½ Penning (Skærv)`, `10 Ducats (Portugaloser)`, `1 Speciedaler (Reichstaler)`, `1 Joachimsdaler (Taler)`, `1 Silver Gulden (Taler)`. → nickname → `note`. NO legend check (nickname is clearly editorial).
- **Modern-currency / metric equivalence:** `1 Øre (0.01 DKK)`, `10 Øre (0.10 DKK)`, `1 Krone = 3/5 Vereinsthaler = 1/50 Metric Pound`. → equivalence → `note`. NO legend check.
- **Genuine two-denomination ambiguity (NEEDS legend verification — which is inscribed?):** `2 Krone (8 Mark)`, `1 Kronerigsdaler (6 Mark)`, `12 Mark (Courant Ducat)`, `12 Mark = 3 Kroner`, `1 Hvid (= 4 Pennings)`, `1 Mark = 16 Skilling`, `1 Reisedaler = 6 Mark`, `12 Rigsdaler courant = 2 Rigsdaler`, `1/15 Speciedaler = 1/12 Rigsdaler Courant`. These need the actual coin legend (IKMK free / danskmoent / Numista — mind the May-2026 API budget) to decide which denomination is the inscription before moving the other to `note`.

**Already done (2026-05-30, commit `3dbdc85`):** the seed-ingest normaliser (`lib/v2_seed_writer.py::_normalise_nominal`) is now head-aware — the translation-hint paren-strip removes ONLY a same-denomination language/plural variant («2 Ducats (Dukaten)» → «2 Ducats»), and KEEPS a cross-denomination equivalent («4 Mark (Krone)» stays) rather than silently deleting it. So the pipeline no longer makes the §1 decision by accident; the existing-data cleanup below is what remains.
...
**Approach (user-chosen 2026-05-30: defer to dedicated session).** When picked up: (1) mechanically move the no-legend-needed sub-patterns (weight / fraction / nickname / modern-equivalence) to `note` / proper fields, leaving the bare inscribed/source denomination as `nominal`; (2) for the two-denomination-ambiguity set, verify the inscription per coin (IKMK first — free), then set `nominal` = inscription and move the equivalent to `note`. Respect V1-frozen (`data/locations/*.yml`) — V2 only. Cross-check against the render-time `normalise_nominal_display` so the cleanup is idempotent with the pipeline.
