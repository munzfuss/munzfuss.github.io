...
+ **Ruler normalization — REMAINING (lower priority):** (a) entity-aware DISPLAY canonicalisation (render German entities' rulers in German form, Danish in Danish — separate from the matching folds, which keep source display form); (b) Christian Albrecht/Christian Albert (Gottorp) + any other German↔English name pairs — same per-entity reign-window method; (c) the §CS propagation (re-merge → absorb → build) of ALL the ruler + nominal + dav dedup fixes, batched.
