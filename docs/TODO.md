...
- **Stage C — REMAINING (220 nominals in V2 final), needs decisions:**
  1. **fraction-of-standard paren (58)** «1 Skilling (1/96 Speciedaler)» + **nickname paren (50)** «1 Speciedaler (Reisedaler)» — leading token is unambiguously the denomination, the paren is editorial. §1 says → `note`. BUT 208 of the dirty coins have NO note → moving requires GENERATING reader-facing note text (role-3 surface, §0a/§2a quality risk). Policy needed: (a) move payload to note (terse, language-neutral append), (b) DROP derivable fractions (build recomputes from fuss per §0) + move only real nicknames, (c) leave for a hand-curated pass.
  2. **dual full-denomination (≈97)** «4 Mark = 1 Krone», «16 Skilling = 1 Mark», «2 Krone (8 Mark)», «12 Mark = 3 Kroner» — which denomination is INSCRIBED? User-flagged as needing legend verification (IKMK free / danskmoent / Numista budget). Leading-primary is a defensible §1 default but the user wants the inscription checked.
  3. **placeholders (11)** «(?)», «Gold coin», «Gold Issue», «Gold Bracteate» + **«Gold Medallic 2 Ducats»** (KM#460, coin-in-medallic-style vs medal) — per-coin research.
  4. **Portugaløser «(N Ducats)» tails** — per the analysis above, nominal = «N Portugaløser», ducat/speciedaler equivalent → note. Mechanizable once the note-policy (1) is decided.
