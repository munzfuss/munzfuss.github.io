...
+ **STACKED ruler-synonym gap (revealed by the dav fix, Gottorp KM 21):** after nominal + dav fixes the pair STILL no_match — ruler «Johann Adolf» (German) ≠ «John Adolphus» (English), same Gottorp duke (grandson, r. 1590-1616). `_normalise_ruler` needs `john adolphus → johann adolf` (exact, so bare «Adolf» = the grandfather duke 1544-1586 stays DISTINCT). **ruler is a PRIMARY discriminator → higher false-fold stakes; curator-directed.** This is the «stacked normalization gaps» pattern: nominal→dav→ruler, each fix revealing the next blocker. Likely more German↔English ruler-form pairs across the Gottorp / Schauenburg German entities (Friedrich/Frederik, Christian Albrecht/Christian Albert, Adolf/Adolphus — each needs a one-person-unambiguous check before adding).
