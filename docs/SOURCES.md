...
*Interim fix (2026-07-14, this session).* Surgical, NOT a parser fix: hand-added `schou: 1-7` + `sieg: '23'` to the galster seed `dk-galster-c3g-131`; dropped the spurious `schou: '1351'` from numismaster seed `schleswig_holstein-numismaster-167746`; set `schou: 1-7` on the seed_unified member + final foundation `unified-dk-bruun-14770` (+ a `_curation_holds: {catalog}` note — NB the catalog hold does NOT survive absorb's main enrich re-derive, only the hygiene-fold; durability here comes from BOTH the foundation and its seed_unified member carrying `1-7`, which absorb unions to a clean `1-7`). These hand edits sit on wholesale-write seeds (galster / numismaster) → a source re-seed reverts them; and a full MERGER re-run would still union Bruun's faithful specimen «Schou 4» with galster «1-7» → `['1-7', '4']` (the «4» is subsumed but the merger has no schou-range-subsumption).
*Proper root fix (deferred — parser session).* Extend `_parse_description_and_refs` to also scan a standalone catalogue parenthetical that follows the `Forside:` block (e.g. widen the block capture past the first blank line up to the spec `<HR>`, or add a second pass that finds `(\s*Galster\s+\d+.*?)` anywhere in the pre-spec text). Then re-parse ALL galster pages (`parse_galster.py --force`) + re-seed galster + review the (broad) diff for other pages that gain Schou/Sieg refs. A merger schou-range-subsumption rule (drop a bare `N` when a covering `A-B` range is present, N ∈ [A,B]) would additionally collapse the Bruun-«4»/galster-«1-7» union to a clean range at merge time.
