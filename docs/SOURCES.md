...
*Proper root fix (deferred — parser session).* Extend `_parse_description_and_refs` to also scan a standalone catalogue parenthetical that follows the `Forside:` block (e.g. widen the block capture past the first blank line up to the spec `<HR>`, or add a second pass that finds `(\s*Galster\s+\d+.*?)` anywhere in the pre-spec text). Then re-parse ALL galster pages (`parse_galster.py --force`) + re-seed galster + review the (broad) diff for other pages that gain Schou/Sieg refs. (No merger rule is needed for the Bruun-«4»/galster-«1-7» union — the existing `normalise_numeric_index` subsumption already collapses it at merge/absorb/render time; see the interim-fix note above.)
